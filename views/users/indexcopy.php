<!DOCTYPE html>
<html lang="es">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Perfil - Diego Anguiano</title>
  <link href="..\..\assets\css\bootstrap.css" rel="stylesheet">
  <link rel="stylesheet" href="..\..\assets\css\style.css">
</head>

<body>
  <nav class="navbar navbar-expand px-4 nav-fija transparente-claro">
    <a class="navbar-brand fw-bold" href="..\home\indexcopy.html">
      <img src="..\..\assets\img\logo.jpg" alt="logo" class="logo" />
    </a>
    <div class="ms-auto d-flex align-items-center">
      <a href="..\users\indexcopy.html" class="btn btn-sm"><i class="fa-solid fa-user"></i> @dam01</a>
      <a href="..\users\logincopy.html" class="btn btn-outline-danger btn-sm"><i
          class="fa-solid fa-arrow-right-from-bracket fa-flip-horizontal"></i> CERRAR SESIÓN</a>
    </div>
  </nav>

  <div class="container mt-4">
    <div class="row columna-fija">
      <aside class="col-md-3 mb-4 menu-fijo">
        <a href="..\users\indexcopy.html" class="text-decoration-none text-white">
          <div class="card mb-3 transparente-oscuro">
            <div class="card-body text-center">
              <h4><i class="fa-solid fa-user"></i> Perfil</h4>
              <br>
              <h5 class="card-title">Diego Anguiano</h5>
              <p class="card-text text-muted">@dam01</p>
              <p class="small">Logroño, España</p>
              <p class="small text-muted">Se unió en marzo de 2025</p>
            </div>
          </div>
        </a>

        <div class="card mb-3 transparente-oscuro">
          <div class="card-body">
            <h6 class="card-title"><i class="fa-solid fa-tags"></i> Etiquetas para ti</h6>
            <span class="badge bg-primary">#futbol</span>
            <span class="badge bg-secondary">#musica</span>
            <span class="badge bg-success">#videojuegos</span>
            <span class="badge bg-warning text-dark">#fornite</span>
          </div>
        </div>

        <div class="d-grid gap-2 transparente-oscuro">
          <a href="..\events\addcopy.html" class="btn btn-success"><i class="fa-solid fa-plus"></i> CREAR EVENTO</a>
          <input class="form-control" placeholder="Buscar Eventos" />
        </div>
      </aside>
      <main class="col-md-9">
        <h4 class="mb-4"><i class="fa-solid fa-thumbtack"></i> Mis Eventos</h4>

        <a href="..\events\indexcopy.html" class="text-decoration-none text-white">
          <div class="card mb-3 transparente-oscuro">
            <div class="card-body">
              <h5>Nombre Evento <small class="text-muted">@usuario • 3h</small></h5>
              <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
              <div class="d-flex justify-content-between">
                <span><i class="fa-solid fa-users"></i> 6 miembros</span>
              </div>
            </div>
          </div>
        </a>

        <a href="..\events\indexcopy.html" class="text-decoration-none text-white">
          <div class="card mb-3 transparente-oscuro">
            <div class="card-body">
              <h5>Otro Evento <small class="text-muted">@ejemplo • 18h</small></h5>
              <p>Descripción de ejemplo para otro evento...</p>
              <div class="d-flex justify-content-between">
                <span><i class="fa-solid fa-users"></i> 4 miembros</span>
              </div>
            </div>
          </div>
        </a>
      </main>
    </div>
  </div>

  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
  <script src="https://kit.fontawesome.com/ae9a063981.js" crossorigin="anonymous"></script>
</body>

</html>